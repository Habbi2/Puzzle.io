...
import { GameState, PuzzlePiece, Player } from './types';

// Game state management
const games = new Map<string, GameState>();
const usernames = new Map<string, string>();
const roomStates = new Map<string, { difficulty: string }>(); // Track game room properties

// Generate a dynamic puzzle with different piece counts based on difficulty
const generatePuzzle = (difficulty = 'medium'): PuzzlePiece[] => {
...
export const setupSocketHandlers = (io: Server): void => {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('move_piece', ({ gameId, pieceId, newPosition }) => {
      ...
      const pieceIndex = game.puzzle.findIndex((piece: PuzzlePiece) => piece.id === pieceId);
      if (pieceIndex !== -1) {
        ...
        const destPieceIndex = game.puzzle.findIndex((piece: PuzzlePiece) => piece.position === newPosition);
        ...
        if (destPieceIndex !== -1) {
          ...
          const isSolved = game.puzzle.every((piece: PuzzlePiece) => piece.position === piece.correctPosition);
    ...
    socket.on('disconnect', () => {
      ...
      for (const [gameId, game] of games.entries()) {
        const playerIndex = game.players.findIndex((player: Player) => player.id === socket.id);
